...
Виджет передаёт актуальный page context с каждым сообщением. Retrieval не
использует клиентский title или сырой URL path как поисковый текст. Он
сопоставляет canonical HTTPS URL только с уже утверждённым активным
источником, добавляет его чанки в кандидаты и применяет ограниченный boost.
Глобальный hybrid ranking остаётся fallback и может вытеснить нерелевантную
текущую страницу.
